...
var Hyperlapse = function(container, params) {
  ...
  var ids = {};
  var last_canvas;
  _loader.on("panorama.load", function(pano) {
    ...
    loaded++;
    console.log("loaded", pano.canvas);
    ids[pano.id].map(hp => (hp.image = pano.canvas || last_canvas));
    last_canvas = pano.canvas || last_canvas;
    if (parsed && _h_points.length == loaded) handleLoadComplete({});
  ...
  var handleLoadComplete = function(e) {
    _is_loading = false;
    _point_index = 0;
    console.log(ids, _h_points.length, Object.keys(ids).length);
    Object.values(ids).map(id =>
      id.map(pano => {
        if (pano.image) return;
        console.log(pano.pano_id);
        let index = pano.index;
        let fallback = null;
        do {
          index++;
          index %= _h_points.length;
          fallback = _h_points[index];
        } while (fallback == null && fallback.image == null);
        pano.image = fallback.image;
      })
    );
  ...
  var parsePoints = function(response) {
    let index = _point_index;
    _loader.load(_raw_points[_point_index], function(point) {
      if (pano.id.startsWith("CAoSLEF")) debugger;
      if (point.id != _prev_pano_id) {
        _prev_pano_id = point.id;

        var hp = new HyperlapsePoint(point.location, point.id, {
          heading: point.rotation,
          pitch: point.pitch,
          elevation: point.elevation,
          copyright: point.copyright,
          image_date: point.image_date
        });
        hp.index = index;
        _h_points.push(hp);

        ids[point.id] = ids[point.id] || [];
        ids[point.id].push(hp);
  ...
  var getElevation = function(locations, callback) {
    ...
    _elevator.getElevationForLocations(positionalRequest, function(
      results,
      status
    ) {
  ...
  var handleDirectionsRoute = function(response) {
    if (!_is_playing) {
      ...
      _d =
        segment_length < _distance_between_points
  ...
  var drawMaterial = function() {
    ...
    if (self.use_lookat)
      _lookat_heading = google.maps.geometry.spherical.computeHeading(
        _h_points[_point_index].location.latLng,
        self.lookat
      );
  ...
  var render = function() {
    if (!_is_loading && self.length() > 0) {
      ...
      var o_x = self.position.x + self.offset.x * t - 35;
      var o_y = self.position.y + self.offset.y * t;
      var o_z = self.tilt + self.offset.z.toRad() * t;

      var o_heading = self.use_lookat
        ? _lookat_heading - _origin_heading.toDeg() + o_x
        : o_x;
  ...
  var download = function(file, filename, type) {
  ...
  var start = new Date().toTimeString();
  var lastIndex = 0;
  var renderIndex = 0;
  ...
  var loop = function() {
    drawMaterial();
    render();
    if (downloading && lastIndex != _point_index) {
      // var context = canvas.getContext("experimental-webgl", {
      //   preserveDrawingBuffer: true
      // });
      let saveFile = index => {
        return data => {
          download(data, `Render${start}-${index}.png`, "image/png");
          renderIndex = lastIndex;
        };
      };

      _renderer.domElement.toBlob(saveFile(_point_index));
      lastIndex = _point_index;
      if (_h_points.length - 1 == _point_index) downloading = false;
      if (lastIndex != renderIndex) return;
    }
  ...
  this.handleLoadComplete = handleLoadComplete;
  ...
  this.load = function() {
    _point_index = 0;
    parsed = true;
    // let pano = _h_points[_point_index];
    // _loader.composePanorama(pano.pano_id);
  };
